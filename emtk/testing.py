"""emtk's own test doubles: a Painter that records, and fixed metrics.
...
emtk is -- ``emtk.testing.RecordingPainter`` -- and the tests import it
from here.
...
def _cache_pixels(cache):
    ...
    ``None`` when there is no cache -- rasterising needs a font library emtk
...
class PixelPainter:
    ...
    Colours are RGBA tuples as everywhere in emtk. Text is blitted from the
    ...
    def text(self, x, y, w, h, align, string, colour, bold=False) -> None:
        ...
        The first is the direction of :attr:`~emtk.font.Atlas.render_scale`:
    ...
    def image(self, x, y, w, h, handle, uv0=(0.0, 0.0), uv1=(1.0, 1.0),
        ...
        ``emtk.painter.image`` calls positionally, so ``handle`` bound to the
        *x coordinate* and every image drew a rectangle at the wrong place.

        A :class:`~emtk.texture.Texture` is rasterised here, nearest
...
def render(gui, size=(0.0, 0.0, 320.0, 200.0), io=None, frames: int = 2):
    ...
    means.
    """
    io = io if io is not None else _emtk.im.IO()
    painter = None
    for _ in range(max(1, frames)):
        painter = PixelPainter(int(size[2]), int(size[3]))
        with _emtk.im.frame(painter, size, io=io):
            gui()
    return painter
