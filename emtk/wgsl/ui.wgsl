// emtk's interface, as quads. One pipeline, one draw call.
//
// This is the toolkit's shader, not any one consumer's. It reads what
// `emtk.quad_painter.QuadPainter` emits and draws the whole interface --
// panels, menus, text, images -- in a single pass. `emtk.wgpu_host` is the
// host built on it, and an application embedding emtk's interface in its own
// render pass loads this same file through `emtk.wgsl`. That is the point of
...
// `emtk.gpu_atlas.image_u`, which is the encoder for this decoder.
//
...
// be that file too. `emtk.wgpu_host` binds an empty group 0 -- the cost of one
