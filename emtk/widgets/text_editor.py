...
Everything draws through :class:`~emtk.painter.Painter`'s six
...
DARK_PALETTE: tuple[Colour, ...] = (
    ...
    # #1e1e1e background; emtk's frame is (50, 50, 56) over a lit scene, and
...
#: The reference's ``GetLightPalette``, same halving. Nothing in emtk selects
...
class Language:
    ...
    def commands(
        ...
        *already* enumerates its verbs. So they are passed in rather than
        ...
        editor without anyone remembering to teach emtk about it.
        ...
        words : iterable of str, optional
            ...
            Coloured as identifiers rather than as verbs. emtk holds no such
...
class Document:
    ...
    def identifiers(self) -> set[str]:
        ...
        There is no autocomplete here, but the emtk command line's completion
...
class TextEditor:
    ...
    *n* is at ``n * text_width("W")``. The chrome's font is Menlo, so this
    ...
    def key(self, key: int, text: str = "", modifiers: int = 0) -> bool:
        ...
        ----------
        key : int
            One of :mod:`emtk.keys`' ``KEY_*`` constants, or ``0`` for a
