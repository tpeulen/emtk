...
that a frame loop refreshes. emtk has neither, so the two are **arguments**:
...
class Selectable:
    ...
    def press(
        ...
        double : bool, optional
            Whether this is the second click of a double click. emtk has no
...
class MultiSelectState:
    ...
    *lower* end works perfectly until the first shift-click above it, at which
    ...
    def click(self, index: int, ctrl: bool = False, shift: bool = False) -> list[int]:
        ...
        ctrl : bool, optional
            Whether ctrl was held. emtk has no modifier feed, so the caller
            says.
        shift : bool, optional
...
class TypingSelect:
    ...
    Notes
    -----
    emtk has no per-frame clock, so ``now`` is an argument to
