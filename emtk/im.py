"""``emtk.im``: Dear ImGui, in Python. Import this one.

The reference is three files and so is this: :mod:`~emtk.im_core` is the
context, the window stack and ``ItemAdd``/``ButtonBehavior`` (``imgui.cpp``);
:mod:`~emtk.im_widgets` is everything built on them
...
def col32(r, g, b, a=255):
    """``IM_COL32``: an RGBA tuple, emtk's colour spelling."""
...
def _published_names():
    ...
    for candidate in (
        _pathlib.Path(__file__).resolve().parents[2] / "tests" / "emtk" / "imgui_api.txt",
    ):
        if candidate.exists():
...
# and emtk's painters take bytes 0..255, so any program transliterated from
...
#: only at import time is a name `emtk.<name>` cannot find.
#: `tests/emtk/test_imgui_api_coverage.py` checks the two agree.
__all__ += [
