...
> host. That was wrong: **cmtk is WGSL-native**. chimol already renders
> cmtk's chrome through wgpu with `ui.wgsl`, and a second shading language
...
**Status: DONE (2026-09-01).** Built as specified. `cmtk/wgpu_host.py`,
`cmtk/wgsl/ui.wgsl`, `cmtk/gpu_atlas.py`, `tests/test_wgpu_host.py`; chimol
loads cmtk's shader. See *What was built* at the foot of this file.
**Repo:** cmtk. Touches `cmtk/wgpu_host.py` (new), `cmtk/font.py`, `tests/`.

## The problem

`cmtk/qt_host.py` does this:
...
rectangle, every glyph, every frame. That is not what cmtk is for. Qt's job
is to open a window and hand wgpu a surface; the drawing belongs on the GPU.

The GPU path already exists and already works: `cmtk/quad_painter.py` is a
...
*inside cmtk*, so any Qt window gets it.
...
`cmtk/wgpu_host.py` — a Qt widget owning a wgpu surface that renders a cmtk
...
another language — move it into cmtk and have chimol use cmtk's copy.**
...
`qt_host` does where Qt or wgpu is absent, so cmtk's zero-dependency
...
5. cmtk's suite stays green, and `tools/gen_names.py --check` is current.

## The one genuinely undesigned piece — decided: port the negative-u

**Image quads.** cmtk has `Texture` and `im.image`, and cmc's image window
...
preserves draw order — text after an image lands *on* it. cmtk's
`QuadPainter` does not encode that today.

**Decided before starting: port the negative-u convention.** It is in
`cmtk/gpu_atlas.py` (`image_u` / `image_texel` / `ImageAtlas`), shared by
...
decay windows) all draw *on top of* the main window, because a cmtk window
created with no explicit box follows the frame box every frame. cmtk has no
...
* **`cmtk/wgsl/ui.wgsl`** — moved out of the GPU viewer, unchanged as code.
  The comment header no longer names an application (cmtk's own
  ...
  owns group 0 compiles the *same file*; `cmtk.wgpu_host` binds an empty
  group 0. `cmtk/wgsl/__init__.py` is the loader and imports nothing.
* **`cmtk/gpu_atlas.py`** — the combined glyph atlas, `ImageAtlas` (now with
  ...
  seconds on cmtk's own atlas. `tests/test_gpu_atlas.py` tests all of it with
  no adapter and no toolkit.
* **`cmtk/wgpu_host.py`** — `WgpuRenderer` (no window, no toolkit; `draw`
...
* **chimol** — `load_wgsl` falls back to `cmtk.wgsl` and its own `ui.wgsl` is
  ...
  prelude plus cmtk's shader compiles.
...
5. **Green.** 1272 passed, 3 skipped with Qt and wgpu present; 1266 passed,
   9 skipped on an interpreter with no Qt at all, which is the claim cmtk
...
what this PRD's correction argued against in the first place. cmtk now has
...
already moved to `cmtk/gpu_atlas.py`, and the tests that were host-agnostic
...
Two things were wrong and both are fixed in `cmtk/im_core.py`:
...
other does not: the box was decided *once* and is cmtk's to remember. A
