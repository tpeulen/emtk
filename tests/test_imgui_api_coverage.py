"""``ImGui::`` → ``emtk.``: the translation is mechanical, and this measures it.

The goal for emtk is that porting a Dear ImGui program needs no thought about
...
``emtk`` answers to it. The count may only go up. A name that is not there yet is
not a failure -- emtk does not claim to be all of Dear ImGui -- but a name that
...
def test_every_imgui_name_translates():
    ...
    missing = [n for n in names if not hasattr(emtk, snake(n))]
    assert not missing, f"{len(missing)} of {len(names)} do not translate: " + \
        ", ".join(f"ImGui::{n} -> emtk.{snake(n)}" for n in missing[:30])
...
def test_nothing_is_a_stub():
    ...
    operation emtk lacked, the painter gained an optional one and emtk falls
    back to something visible instead.
    """
    names = _names("imgui_api.txt") + _names("imdrawlist_api.txt")
    refusing = [n for n in names
                if getattr(getattr(emtk, snake(n), None), "emtk_unsupported", False)]
...
def test_the_widgets_a_port_reaches_for_first_are_all_there():
    ...
    for name in ("Begin", "End", "Text", "TextColored", "TextDisabled",
        ...
        assert hasattr(emtk, snake(name)), f"ImGui::{name} -> emtk.{snake(name)}"


def test_the_package_and_the_module_agree():
    """`emtk.button` and `emtk.im.button` are the same function.
    ...
    for name in _names("imgui_api.txt"):
        attr = snake(name)
        assert getattr(emtk, attr) is getattr(im, attr), attr
