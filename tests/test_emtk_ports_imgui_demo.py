"""``imgui_demo.cpp``, ported. What the port cannot express is a emtk defect.

emtk exists so that Dear ImGui code can be brought to Python and drawn on GL.
...
needed and emtk lacked was added because *this* asked for it, which is a better
...
What the port needed and emtk did not have, all now present:
...
def widgets_basic(state: DemoState, io: emtk.IO, storage: dict) -> RecordingPainter:
    """``ShowDemoWindowWidgets``'s "Basic" section, line for line."""
    painter = RecordingPainter()
    with emtk.frame(painter, (0.0, 0.0, 420.0, 700.0), io=io, storage=storage) as ctx:
        emtk.begin("Dear ImGui Demo")
        ...
        if state.clicked & 1:
            emtk.same_line()
            emtk.text("Thanks for clicking me!")

        # ImGui::Checkbox("checkbox", &check);
        _, state.check = emtk.checkbox("checkbox", state.check)

        # ImGui::RadioButton("radio a", &e, 0); ImGui::SameLine();
        # ImGui::RadioButton("radio b", &e, 1); ImGui::SameLine();
        # ImGui::RadioButton("radio c", &e, 2);
        _, state.e = emtk.radio_button("radio a", state.e, 0)
        emtk.same_line()
        _, state.e = emtk.radio_button("radio b", state.e, 1)
        emtk.same_line()
        _, state.e = emtk.radio_button("radio c", state.e, 2)
        ...
        for i in range(7):
            if i > 0:
                emtk.same_line()
            emtk.push_id(i)
            emtk.push_style_color(emtk.Col.BUTTON, _hsv(i / 7.0, 0.6, 0.6))
            emtk.push_style_color(emtk.Col.BUTTON_HOVERED, _hsv(i / 7.0, 0.7, 0.7))
            emtk.push_style_color(emtk.Col.BUTTON_ACTIVE, _hsv(i / 7.0, 0.8, 0.8))
            emtk.button("Click")
            emtk.pop_style_color(3)
            emtk.pop_id()

        # ImGui::AlignTextToFramePadding();
        # ImGui::Text("Hold to repeat:"); ImGui::SameLine();
        emtk.align_text_to_frame_padding()
        emtk.text("Hold to repeat:")
        emtk.same_line()
        ...
        spacing = emtk.get_style().item_inner_spacing[0]
        emtk.push_item_flag(emtk.ItemFlags.BUTTON_REPEAT, True)
        if emtk.arrow_button("##left", emtk.Dir.LEFT):
            state.counter -= 1
        state.boxes["##left"] = ctx.get_item_rect()   # for the test below
        emtk.same_line(0.0, spacing)
        if emtk.arrow_button("##right", emtk.Dir.RIGHT):
            state.counter += 1
        emtk.pop_item_flag()
        emtk.same_line()
        emtk.text("%d" % state.counter)
        ...
        emtk.label_text("label", "Value")

        # ImGui::SeparatorText("Inputs");
        emtk.separator_text("Inputs")
        ...
        _, state.i = emtk.input_int("input int", state.i)

        emtk.separator_text("Drags")
        # ImGui::DragFloat("drag float", &f1, 0.005f);
        _, state.f = emtk.drag_float("drag float", state.f, 0.005)
        # ImGui::DragInt("drag int", &i1, 1);
        _, state.i = emtk.drag_int("drag int", state.i, 1)

        emtk.separator_text("Sliders")
        # ImGui::SliderInt("slider int", &i1, -1, 3);
        _, state.i = emtk.slider_int("slider int", state.i, -1, 3)
        # ImGui::SliderFloat("slider float", &f1, 0.0f, 1.0f, "ratio = %.3f");
        _, state.f = emtk.slider_float("slider float", state.f, 0.0, 1.0, "ratio = %.3f")
        # ImGui::SliderAngle("slider angle", &angle);
        _, state.angle = emtk.slider_angle("slider angle", state.angle)

        emtk.separator_text("Selectors/Pickers")
        # ImGui::ColorEdit3("color 1", (float*)&color);
        _, state.col = emtk.color_edit3("color 1", state.col)
        # ImGui::CheckboxFlags("flag", &flags, 1 << 0);
        _, state.flags = emtk.checkbox_flags("flag", state.flags, 1 << 0)
...
def test_the_style_colour_stack_unwinds(demo):
    ...
    with emtk.frame(painter, (0.0, 0.0, 200.0, 100.0)) as ctx:
        before = ctx.style.color(emtk.Col.BUTTON)
        emtk.push_style_color(emtk.Col.BUTTON, (1, 2, 3, 255))
        emtk.push_style_color(emtk.Col.BUTTON_HOVERED, (4, 5, 6, 255))
        emtk.push_style_color(emtk.Col.BUTTON_ACTIVE, (7, 8, 9, 255))
        assert ctx.style.color(emtk.Col.BUTTON) == (1, 2, 3, 255)
        emtk.pop_style_color(3)
        assert ctx.style.color(emtk.Col.BUTTON) == before
...
def test_push_id_keeps_the_seven_buttons_apart(demo):
    ...
    """
    painter = RecordingPainter()
    with emtk.frame(painter, (0.0, 0.0, 400.0, 100.0)) as ctx:
        ids = []
        for i in range(7):
            emtk.push_id(i)
...
def im_flags(flags: int, bit: int) -> tuple[bool, int]:
    ...
    for phase in ("place", "press", "release"):
        painter = RecordingPainter()
        with emtk.frame(painter, (0.0, 0.0, 200.0, 100.0), io=io, storage=storage) as ctx:
            changed, flags = emtk.checkbox_flags("flag", flags, bit)
