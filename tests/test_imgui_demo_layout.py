...
def _run(gui, size=(0.0, 0.0, 500.0, 400.0), io=None, storage=None):
    ...
    io = io if io is not None else cmtk.IO()
    with cmtk.frame(painter, size, io=io, storage=storage if storage is not None else {}):
        gui()
    return painter
...
def test_same_line_puts_the_next_item_beside_the_last():
    ...
    def gui():
        cmtk.text("Two items: Hello")
        boxes["hello"] = cmtk.get_item_rect()
        cmtk.same_line()
        cmtk.text_colored((255, 255, 0, 255), "Sailor")
        boxes["sailor"] = cmtk.get_item_rect()
...
def test_same_line_takes_a_spacing():
    ...
    def gui():
        cmtk.text("More spacing: Hello")
        boxes["hello"] = cmtk.get_item_rect()
        cmtk.same_line(0.0, 20.0)
        cmtk.text_colored((255, 255, 0, 255), "Sailor")
        boxes["sailor"] = cmtk.get_item_rect()
...
def test_same_line_takes_an_absolute_offset():
    ...
    def gui():
        cmtk.text("Aligned")
        cmtk.same_line(150.0)
        cmtk.text("x=150")
        boxes["150"] = cmtk.get_item_rect()
        cmtk.same_line(300.0)
        cmtk.text("x=300")
        boxes["300"] = cmtk.get_item_rect()
...
def test_a_dummy_takes_up_room_and_draws_nothing():
    ...
    def gui():
        cmtk.button("A")
        boxes["A"] = cmtk.get_item_rect()
        cmtk.same_line()
        boxes["gap"] = cmtk.dummy(40.0, 20.0)
        cmtk.same_line()
        cmtk.button("B")
        boxes["B"] = cmtk.get_item_rect()
...
def test_set_next_item_width_applies_to_exactly_one_item():
    ...
    def gui():
        cmtk.set_next_item_width(120.0)
        cmtk.slider_float("narrow", 0.5, 0.0, 1.0)
        boxes["narrow"] = cmtk.get_item_rect()
        cmtk.slider_float("wide", 0.5, 0.0, 1.0)
        boxes["wide"] = cmtk.get_item_rect()
...
def test_push_item_width_applies_until_it_is_popped():
    ...
    def gui():
        cmtk.push_item_width(90.0)
        assert cmtk.calc_item_width() == pytest.approx(90.0)
        cmtk.pop_item_width()
...
def test_a_child_region_lays_out_inside_its_box():
    ...
    def gui():
        cmtk.begin_child((10.0, 10.0, 200.0, 120.0))
        cmtk.text("inside")
        boxes["inside"] = cmtk.get_item_rect()
        cmtk.end_child()
        cmtk.text("outside")
        boxes["outside"] = cmtk.get_item_rect()
...
def test_a_child_region_clips_what_overflows_it():
    ...
    def gui():
        cmtk.begin_child((0.0, 0.0, 200.0, 60.0))
        for index in range(100):
            cmtk.text("%04d: scrollable region" % index)
        cmtk.end_child()
...
def test_text_wrapped_breaks_into_more_than_one_line():
    ...
    def gui():
        ...
        boxes.append(cmtk.get_item_rect())
...
def test_scroll_is_clamped_to_what_there_is_to_scroll():
    def gui():
        cmtk.begin("scroller", (0.0, 0.0, 100.0, 100.0))
        cmtk.set_scroll_y(999.0)
        cmtk.end()

    painter = RecordingPainter()
    with cmtk.frame(painter, (0.0, 0.0, 400.0, 400.0)) as ctx:
        ctx.begin("scroller", (0.0, 0.0, 100.0, 100.0))
        ctx.current_window.content_size = (100.0, 250.0)
        cmtk.set_scroll_here_y(1.0)
        assert cmtk.get_scroll_y() == pytest.approx(cmtk.get_scroll_max_y())
        assert cmtk.get_scroll_y() == pytest.approx(150.0)
        ctx.end()
