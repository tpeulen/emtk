...
Each needed something cmtk did not have, and each of those is now built:
...
* **docking** -- a dock node is a box windows share, and windows sharing one
  ...
  *makes* a node belongs to the host's window frame, so a host that drags calls
  ``set_next_window_dock_id`` and cmtk does the rest.
...
def test_two_windows_can_share_a_title_and_stay_apart():
    ...
    def gui():
        cmtk.begin("Same title as another window##1", (0.0, 0.0, 100.0, 50.0))
        cmtk.text("This is window 1.")
        cmtk.end()
        cmtk.begin("Same title as another window##2", (0.0, 60.0, 100.0, 50.0))
        cmtk.text("This is window 2.")
        cmtk.end()
        ctx = cmtk.get_current_context()
...
def test_a_title_that_changes_every_frame_keeps_one_window():
    ...
    def gui():
        ...
        cmtk.begin("Animated title %s %d###AnimatedTitle"
                   % (spinner, ctx.io.frame_count), (0.0, 0.0, 120.0, 40.0))
        cmtk.text("This window has a changing title.")
        cmtk.end()
...
def test_windows_docked_into_one_node_become_its_tabs():
    ...
    def gui():
        dock = cmtk.dock_space_over_viewport(1)
        for name in ("Lettre", "Farrago", "Abstract"):
            if cmtk.begin_docked(name, dock):
                cmtk.text("body of %s" % name)
            cmtk.end_docked()
            seen.setdefault("tabs", {})[name] = cmtk.get_item_rect()
        node = cmtk.get_current_context().dock_node(1)
...
def test_clicking_a_dock_tab_brings_that_document_forward():
    ...
    def gui():
        dock = cmtk.dock_space_over_viewport(1)
        for name in ("Lettre", "Farrago", "Abstract"):
            on_top = cmtk.begin_docked(name, dock)
            if on_top:
                cmtk.text("body of %s" % name)
            cmtk.end_docked()
        ctx = cmtk.get_current_context()
    ...
    def probe():
        ...
        node = cmtk.get_current_context().dock_node(1)
        x, y, _w, _h = node["box"]
        height = cmtk.get_frame_height()
        offset = x
        for name in ("Lettre", "Farrago", "Abstract"):
            width = cmtk.calc_text_size(name)[0] + cmtk.get_style().frame_padding[0] * 2
...
def test_a_dock_node_can_be_split_in_two():
    ...
    def gui():
        dock = cmtk.dock_space_over_viewport(1)
        left, right = cmtk.dock_builder_split_node(dock, cmtk.Dir.LEFT, 0.25)
        ctx = cmtk.get_current_context()
...
def test_a_window_says_whether_it_is_docked():
    ...
    def gui():
        dock = cmtk.dock_space_over_viewport(1)
        cmtk.begin_docked("Docked", dock)
        seen["docked"] = cmtk.is_window_docked()
        seen["id"] = cmtk.get_window_dock_id()
        cmtk.end_docked()
        cmtk.begin("Floating", (0.0, 200.0, 100.0, 50.0))
        seen["floating"] = cmtk.is_window_docked()
        cmtk.end()
...
def test_a_window_can_be_docked_before_it_is_drawn():
    ...
    def gui():
        cmtk.dock_space_over_viewport(1)
        cmtk.dock_builder_dock_window("Preplaced", 1)
        cmtk.begin_docked("Preplaced")
        seen["id"] = cmtk.get_window_dock_id()
        cmtk.end_docked()
...
def test_a_dock_space_fills_the_box_it_was_given():
    ...
    def gui():
        cmtk.dock_space_over_viewport(1)
        seen["node"] = cmtk.get_current_context().dock_node(1)["box"]
        seen["viewport"] = cmtk.get_main_viewport()
...
def _fields_gui(state):
    def gui():
        cmtk.process_nav_keys()
        for index in range(3):
            _c, state["text%d" % index] = cmtk.input_text(
                "%d" % index, state.get("text%d" % index, ""))
            state["id%d" % index] = cmtk.get_item_id()
        state["nav"] = cmtk.get_nav_id()
        state["ring"] = cmtk.get_nav_ring()
    return gui
...
def test_tab_moves_the_focus_from_one_field_to_the_next():
    ...
    frames.io.config_flags = cmtk.ConfigFlags.NAV_ENABLE_KEYBOARD
    frames.draw()
...
def test_focus_can_be_placed_from_code():
    ...
    def gui():
        cmtk.text("before")
        if state["want"]:
            cmtk.set_keyboard_focus_here()
            state["want"] = False
        _c, state["text"] = cmtk.input_text("field", state.get("text", ""))
        state["id"] = cmtk.get_item_id()
        state["nav"] = cmtk.get_nav_id()
...
def test_a_shortcut_fires_only_on_its_own_chord():
    ...
    def gui():
        ...
        seen["ctrl_a"] = cmtk.shortcut(chord)
        seen["plain_a"] = cmtk.shortcut(keys.KEY_A)
...
def test_a_shortcut_can_be_claimed_for_the_next_item():
    def gui():
        cmtk.set_next_item_shortcut(keys.MOD_CTRL | keys.KEY_S)
        cmtk.button("Save")
        assert cmtk.get_current_context().state(("shortcut",))["next"] == \
...
def test_live_edit_decides_when_a_drag_reports_its_change():
    ...
    def gui(live):
        cmtk.push_item_flag(cmtk.ItemFlags.LIVE_EDIT_ON_INPUT, live)
        changed, _v = cmtk.drag_float("value", 1.0, 1.0)
        boxes["drag"] = cmtk.get_item_rect()
        cmtk.pop_item_flag()
...
def test_two_multi_select_scopes_keep_their_own_selections():
    ...
    def gui():
        cmtk.push_id("left")
        cmtk.begin_multi_select(0, len(left), 3)
        for n in range(3):
            if cmtk.selectable("L%d" % n, n in left):
                left.symmetric_difference_update({n})
            boxes[("left", n)] = cmtk.get_item_rect()
        io_left = cmtk.end_multi_select()
        cmtk.pop_id()

        cmtk.push_id("right")
        cmtk.begin_multi_select(0, len(right), 3)
        for n in range(3):
            if cmtk.selectable("R%d" % n, n in right):
                right.symmetric_difference_update({n})
            boxes[("right", n)] = cmtk.get_item_rect()
        io_right = cmtk.end_multi_select()
        cmtk.pop_id()
...
def test_the_backend_flags_describe_the_painter_in_hand():
    ...
    def gui(into):
        into["flags"] = cmtk.get_io().backend_flags

    _run(lambda: gui(plain))
    assert not plain["flags"] & cmtk.BackendFlags.RENDERER_HAS_IMAGES
    assert plain["flags"] & cmtk.BackendFlags.HAS_MOUSE_CURSORS
    ...
    painter = Capable()
    with cmtk.frame(painter, (0.0, 0.0, 100.0, 100.0)):
        gui(rich)
    assert rich["flags"] & cmtk.BackendFlags.RENDERER_HAS_IMAGES
    assert rich["flags"] & cmtk.BackendFlags.RENDERER_HAS_FONTS
    assert rich["flags"] & cmtk.BackendFlags.RENDERER_HAS_ROTATED_TEXT
...
def test_the_config_flags_are_the_applications_to_set():
    ...
    def gui():
        seen["nav"] = bool(cmtk.get_io().config_flags
                           & cmtk.ConfigFlags.NAV_ENABLE_KEYBOARD)
        seen["capture"] = (cmtk.get_io().want_capture_mouse,
                           cmtk.get_io().want_capture_keyboard)

    io = cmtk.IO()
    io.config_flags = cmtk.ConfigFlags.NAV_ENABLE_KEYBOARD
    _run(gui, io=io)
...
def test_want_capture_reports_whether_the_gui_took_the_input():
    ...
    def gui():
        cmtk.begin("w", (0.0, 0.0, 100.0, 100.0))
        cmtk.button("b")
        cmtk.end()
