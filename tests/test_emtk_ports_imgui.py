"""A Dear ImGui program ports to emtk line for line.
...
Line for line, that is::
    ...
    _, show_demo_window = emtk.checkbox("Demo Window", show_demo_window)
    _, show_another_window = emtk.checkbox("Another Window", show_another_window)
    _, f = emtk.slider_float("float", f, 0.0, 1.0)
    if emtk.button("Button"):
        counter += 1
    emtk.same_line()
    emtk.text("counter = %d" % counter)
    emtk.end()
...
def _hello_world(state: dict) -> RecordingPainter:
    ...
    with emtk.frame(painter, (0.0, 0.0, 300.0, 200.0), io=state["io"],
                  storage=state["storage"]):
        emtk.begin("Hello, world!")
        emtk.text("This is some useful text.")
        _, state["show_demo_window"] = emtk.checkbox(
            "Demo Window", state["show_demo_window"])
        _, state["show_another_window"] = emtk.checkbox(
...
def test_the_cursor_advances_and_same_line_holds_it(state):
    ...
    *item* is.
    ...
    with emtk.frame(painter, (0.0, 0.0, 300.0, 200.0), io=state["io"],
        ...
        emtk.checkbox("Demo Window", True)
        boxes.append(("checkbox", ctx.get_item_rect()))
        emtk.slider_float("float", 0.0, 0.0, 1.0)
...
def test_the_item_under_the_pointer_is_the_one_that_was_drawn_there(state):
    ...
    painter = RecordingPainter()
    with emtk.frame(painter, (0.0, 0.0, 300.0, 200.0), io=io,
                  storage=state["storage"]) as ctx:
        emtk.begin("Hello, world!")
        emtk.text("This is some useful text.")
        emtk.checkbox("Demo Window", state["show_demo_window"])
        emtk.checkbox("Another Window", state["show_another_window"])
        emtk.slider_float("float", state["f"], 0.0, 1.0)
        emtk.button("Button")
        hovered_the_button = emtk.is_item_hovered()
        emtk.end()
...
def test_only_one_item_claims_the_pointer(state):
    ...
    with emtk.frame(painter, (0.0, 0.0, 200.0, 100.0), io=io) as ctx:
...
def test_the_window_list_is_walked_backwards_to_find_the_hovered_one(state):
    ...
    painter = RecordingPainter()
    io = emtk.IO()
    with emtk.frame(painter, (0.0, 0.0, 200.0, 200.0), io=io) as ctx:
