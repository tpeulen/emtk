...
:mod:`cmtk.widgets.plot` already draws the picture: axes that auto-fit, ticks,
...
C++ did. Unlike the reference, nothing is drawn until ``end_plot`` -- cmtk's
...
def end_plot() -> None:
    ...
    if _cur.y_label:
        # ImPlot rotates this up the left edge; not every cmtk painter can
...
def setup_axis_limits(axis: int, vmin: float, vmax: float,
    ...
    """Pin an axis. cmtk rebuilds the plot every frame, so there is no
...
def setup_legend(loc: int = 0, flags: int = 0) -> None:
    """Accepted and ignored: cmtk's Plot places its legend itself."""


# -- style ----------------------------------------------------------------- #
def _rgba(colour) -> tuple:
    """ImPlot styles in floats 0..1; cmtk paints in bytes 0..255."""
    if colour is None:
        return None
    vals = list(colour)
...
def plot_bars(label: str, xs, *args, ys=None, count: int | None = None,
    ...
    Bars are drawn as a step outline. cmtk's Plot has no filled-rectangle
...
def plot_shaded(label: str, xs, ys1, ys2=None, count: int | None = None) -> None:
    """The band's two edges, as lines. cmtk's Plot has no filled area; the
