...
This is cmtk's node editor. Like the rest of cmtk it is immediate mode -- you
call :func:`begin_node` every frame and the editor remembers where the node was
dragged to -- and like the rest of cmtk it is a **re-implementation**, written
...
inside a node do not**. That is not a shortcut, it is what cmtk can currently
...
size-aware atlas in cmtk, which is its own piece of work.
...
#: ``(r, g, b, a)`` bytes -- cmtk's convention, not ImGui's floats. Writing a
...
class Canvas:
    ...
    of any size is unreadable without it and cmtk's fonts take a size at
...
def _pointer_over_editor(box: tuple) -> bool:
    ...
    saying why. An editor is a ``begin_child``, and in cmtk a child is a
