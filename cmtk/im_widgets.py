"""``cmtk.im_widgets``: Dear ImGui's widget set, spelled for Python.
...
top of them. :mod:`cmtk.im` (the facade) re-exports these, so a port reads the way the
original does::
...
through the :class:`~cmtk.drawlist.DrawList`. Because the box registered
...
# `imgui_demo.cpp`'s "Widgets/Basic" section asked for it and cmtk had no
...
# `cmtk.keys` spellings, imported here so the module keeps its own dependency.
...
class _Table:
    ...
    cmtk's tables used to be the layout's equal columns with a stack on top,
...
def text_link_open_url(label: str, url: str = "") -> None:
    ...
    cmtk draws and reports; it does not reach for a browser. The click is
...
def get_version() -> str:
    """``ImGui::GetVersion``: cmtk's, since that is what is running."""
...
# --------------------------------------------------------------------------- #
#
# The last of the published API. Each is implemented in cmtk's terms rather
...
# settings, the log and the clipboard are things cmtk can hold perfectly well
# on its own.
...
def get_font_baked(size: float = 0.0):
    """``ImGui::GetFontBaked``. cmtk bakes nothing; the font is the font."""
    return get_font()
...
# -- the platform: the box cmtk was handed ---------------------------------- #
class Viewport:
...
class PlatformIO:
    """``ImGuiPlatformIO``: the host hooks cmtk actually has."""
...
def get_clipboard_text() -> str:
    """``GetClipboardText``. The host may hook it; cmtk keeps one otherwise."""
    ctx = get_current_context()
...
def set_drag_drop_payload(kind: str, data, cond: int = 0) -> bool:
    """``SetDragDropPayload``: any Python object; cmtk does not copy bytes."""
...
def debug_log(s: str) -> None:
    """``DebugLog``: cmtk's own log, shown by `show_debug_log_window`."""
...
def show_about_window(open_: bool = True) -> None:
    ...
    if not open_:
        return
    text("cmtk -- Dear ImGui, in Python, on any painter")
...
def show_demo_window(open_: bool = True) -> None:
    """``ShowDemoWindow``: the reference's demo, in the widgets cmtk has.
    ...
    text("cmtk %s -- Dear ImGui in Python" % get_version())
...
# --------------------------------------------------------------------------- #
#
# The half of docking that is cmtk's: a node is a box that windows share, and
...
# that drags calls `set_next_window_dock_id` and cmtk does the rest.
