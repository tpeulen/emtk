...
parse, and where their meaning in cmtk lives. Two shapes:

* **Flag families** are classes of small ints, so ``|`` and ``&`` behave as
  the C++ did. The bit values are cmtk's own (assigned in the order Dear
  ImGui declares them); a port that only ORs, ANDs and tests them never
  notices.
* ``StyleVar`` members name attributes of :class:`~cmtk.Style` -- that is
  what ``PushStyleVar`` pushes in cmtk.
* ``Key`` members carry :mod:`cmtk.keys` values, which is what ``io.key``
  delivers.
* ``DataType`` members are strings, because cmtk's scalar widgets dispatch
  on the Python type of the value.

A bit a widget does not implement is a widget's business: cmtk functions
...
def _bits(*names: str) -> type:
    ...
    Members are spelled the way :func:`cmtk.im_compat.mechanical_name`
...
ColorEditFlags = _bits(
    ...
    # cmtk's colour widgets take these and implement what they implement --
...
InputTextFlags = _bits(
    ...
    # cmtk implements (im.input_text): CharsHexadecimal, CharsDecimal,
...
SliderFlags = _bits(
    ...
    # cmtk implements: none of the behaviour bits yet; drag_scalar/slider_scalar
...
Key = _named(
    ...
    # a shortcut bound to one silently never fired. A code cmtk already had.
    ...
    # These four have no code of their own: they are *modifiers*, and cmtk
