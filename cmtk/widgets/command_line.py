...
-------------------------------
cmtk's typed interface used to exist in exactly one place: a console docked
...
``keydown`` listener, both translating into :mod:`cmtk.keys` first.
...
#: in a terminal, so it has to declare what it is a prompt *for*. cmtk does not
...
class CommandLine:
    ...
    run : callable, optional
        ...
        else that can run a cmtk command passes its own.
    """
    ...
    def key(self, key: int, text: str = "", modifiers: int = 0) -> bool:
        ...
        ----------
        key : int
            One of :mod:`cmtk.keys`' constants, or ``0`` for a key that
            only produces text.
        text : str
