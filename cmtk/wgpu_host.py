...
was the consumer *inside cmtk*, so that any Qt window gets it. That is this
module.
...
---------------------
cmtk is **WGSL-native**. ``wgsl/ui.wgsl`` is the toolkit's shader, and it is
the *same file* a wgpu application embedding cmtk loads -- a second shading
...
``import cmtk.wgpu_host`` in a headless process is as cheap and as harmless
as importing :mod:`.qt_host` there.

Bind groups
-----------
The shader's bindings are **group 1**, and group 0 is left empty. That is
not an oversight. An application that embeds cmtk's interface in its own
...
def _wgpu():
    ...
    ImportError
        ...
        :func:`.qt_host.host_class` gives without Qt. cmtk's claim is that
...
def default_device():
    ...
    if adapter is None:  # pragma: no cover - a machine with no GPU at all
        raise RuntimeError(
            "no wgpu adapter on this machine; cmtk.qt_host is the fallback"
        )
...
class WgpuRenderer:
    ...
    Parameters
    ----------
    atlas : cmtk.font.Atlas, optional
    ...
    def _shader(self):
        ...
        if self._module is None:
            self._module = self.device.create_shader_module(
                label="cmtk.ui", code=load_wgsl(UI_SHADER)
            )
        return self._module
                ...
                "buffers": [
                    {
                        ...
                        # :data:`cmtk.gpu_atlas.ATTRIBUTES`, which is the
    ...
    def render(self, vertices, width: float, height: float, view,
        ...
        """
        device = self.device
        encoder = device.create_command_encoder(label="cmtk.ui")
        attachment = {
            "view": view,
            "resolve_target": None,
...
def _clear_value(background) -> tuple:
    ...
    Colours are **bytes** in cmtk and floats in wgpu. Handing 0-1 floats
...
def _render_widget():
    ...
    so in a sentence that reads like a bug in cmtk. Importing ``qtpy``
    first is what picks the binding the rest of cmtk is already using.
    """
...
def wgpu_host_class():
    ...
    class _WgpuControlHost(QRenderWidget):
        ...
        def _configure(self):
            ...
            Notes
            -----
            The format is the **surface's** preferred one, not cmtk's: a
